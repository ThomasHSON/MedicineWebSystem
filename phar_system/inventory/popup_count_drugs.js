...
function isMobileOrTablet() {
  ...
  const mobileKeywords =
    /Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini|Tablet|Windows Phone|Kindle|Silk|Mobile|PlayBook/i;
...
function renderResult(data) {
  ...
  console.log("渲染點點資料型態陣列：", Array.isArray(data.Data));
  ...
  data.Data.forEach((item, index) => {
    ...
    item.value.forEach((value) => {
      ...
      div.style.left = `${
        (centerX / 960) * container.offsetWidth -
        div.style.width.replace("px", "") / 2
      }px`;
      div.style.top = `${
        (centerY / 960) * container.offsetHeight -
        div.style.height.replace("px", "") / 2
      }px`;
...
function performance_result() {
  ...
  if (handle_time) {
    let ppcd_perf_result_time = document.querySelector(
      ".ppcd_perf_result_time"
    );
