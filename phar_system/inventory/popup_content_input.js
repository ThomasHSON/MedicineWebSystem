...
async function show_popup_input(Content, page_Initial, show_all_user) {
  ...
  if (show_all_user == undefined) {
    ...
    if (urlParams.has("administrator")) {
      ...
      popup_input_div_SubContent = Content.Sub_content.filter(
        function (subItem) {
  ...
  popup_input_MaxfPage = Math.floor(
    popup_input_div_SubContent.length / popup_input_NumOfPageRows,
  );
  if (popup_input_div_SubContent.length % popup_input_NumOfPageRows > 0)
    popup_input_MaxfPage++;
...
function notice_popup_sub_result(msg, status) {
  let popup_notice_for_sub_container = document.querySelector(
    ".popup_notice_for_sub_container",
  );
  ...
  setTimeout(notice_control, 5000);
}
...
function get_popup_input() {
  popup_input_div = new Basic_popup_Div(
    "popup_input_div_popup_input",
    "popup_input_div_popup_input",
    "340px",
    "",
  );
...
function edit_title_popup_input(Content) {
  ...
  const med_SKDIACODE_text = document.querySelector(
    "#med_SKDIACODE_text_popup_input",
  );
  ...
  const med_eng_name_text = document.querySelector(
    "#med_eng_name_text_popup_input",
  );
  if (Content.NAME != null)
    med_eng_name_text.innerText = `(英) : ${Content.NAME}`;
  const med_cht_name_text = document.querySelector(
    "#med_cht_name_text_popup_input",
  );
  if (Content.CHT_NAME != null)
    med_cht_name_text.innerText = `(中) : ${Content.CHT_NAME}`;

  const med_end_QTY_text = document.querySelector(
    "#med_end_QTY_text_popup_input",
  );
  med_end_QTY_text.innerText = `盤點量 : \n${+Content.END_QTY}`;
  console.log(Content);
  const med_end_PKG_text = document.querySelector(
    "#med_end_PKG_text_popup_input",
  );
  med_end_PKG_text.innerText = `單位 : \n${Content.PAKAGE}`;

  let med_storage_popup_block = document.querySelector(
    ".med_storage_popup_block",
  );
  med_storage_popup_block.innerHTML = `儲位：${Content.STORAGE_NAME}`;
  console.log(`儲位：${Content.STORAGE_NAME}`);
}
function edit_rows_popup_input(Content) {
  ...
  var end_index =
    popup_input_PageIndex * popup_input_NumOfPageRows +
    popup_input_NumOfPageRows;
  while (true) {
...
function edit_rows_page_control_popup_input() {
  if (popup_input_MaxfPage == 0) {
    const rows_page_control_block = document.querySelector(
      "#rows_page_control_block_popup_input",
    );
    ...
    if (popup_input_PageIndex + 1 < popup_input_MaxfPage)
      svg_next.style.visibility = "visible";
    else svg_next.style.visibility = "hidden";
    if (popup_input_PageIndex - 1 >= 0)
      svg_previous.style.visibility = "visible";
...
function get_title_popup_input() {
  ...
  My_Div.Init(
    med_CODE_SKDIACODE_block,
    "med_CODE_SKDIACODE_block_popup_input",
    "med_CODE_SKDIACODE_block_popup_input",
    "100%",
    "20%",
    "",
  ...
  let med_storage_popup_block = document.createElement("div");
  med_storage_popup_block.classList.add("med_storage_popup_block");
  med_storage_popup_block.innerHTML = "儲位：";

  med_info.appendChild(med_CODE_SKDIACODE_block);
  med_info.appendChild(med_storage_popup_block);
...
function get_underline_popup_input() {
  ...
  calculate_Back_btn.addEventListener("click", function () {
    popup_input_END_QTY_input.value = popup_input_END_QTY_input.value.slice(
      0,
      -1,
    );
  });
  ...
  calculate_Result_btn.addEventListener("click", function () {
    popup_input_END_QTY_input.value = calculateExpression(
      popup_input_END_QTY_input.value,
    );
  });
...
function get_block_popup_input(Sub_content) {
  ...
  My_Div.Init(
    block2_Value_text_popup_input,
    "block2_Value_text_popup_input",
    "block1_Value_text_popup_input",
    "100%",
    "",
    "",
  );
        ...
        if (confirm(confirm_msg)) {
          ...
          if (confirm(cancel_msg)) {
            temp_arr = temp_arr.filter(
              (item) => !cancel_GUID_arr.includes(item.GUID),
            );
