...
function refresh_Header_state() {
  if (
    !popup_creatSelect_div.Get_Visible() &&
    !popup_input_div.Get_Visible() &&
    !popup_med_serch_div.Get_Visible()
  )
...
setInterval(async function () {
  if (current_IC_SN != "" && current_IC_SN != null) {
    ...
    let return_data = await report_member(
      "盤點操作",
      current_IC_SN,
      userID,
      userName
...
async function init() {
  ...
  set_inventory_info_to_iframe(data);
...
async function Refresh_rows() {
  ...
  const data_temp = data.Data[0].Contents.filter(function (constent) {
    return constent.Sub_content.length > 0;
  });
...
function get_header() {
  ...
  let header_med_map = document.createElement("button");
  header_med_map.classList.add("header_med_map");
  header_med_map.innerHTML = "地圖模式";
  header_med_map.addEventListener("click", () => {
    trgger_iframe_func(true);
  });
  ...
  header_controls.appendChild(header_med_map);
  ...
  //搜尋列
  let header_search_text_container = document.createElement("div");
  header_search_text_container.classList.add("header_search_text_container");
  ...
  header_search_text_img_icon.classList.add("header_search_text_img_icon");
    ...
    switch (event.data.type) {
      ...
      case "MED_CLICKED":
        ...
        for (var i = 0; i < data.Data[0].Contents.length; i++) {
          ...
          if (CODE.toUpperCase() == event.data.code.toUpperCase()) {
...
function set_inventory_info_to_iframe(data) {
  data.Data[0].Contents.forEach((element) => {
    data_object[`med_${element.CODE}`] = { END_QTY: element.END_QTY };
  });
