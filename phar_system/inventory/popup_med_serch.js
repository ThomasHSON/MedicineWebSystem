...
async function popup_med_serch_init() {
  popup_med_serch_div = new Basic_popup_Div(
...
function popup_med_serch_content_init() {
  ...
  const content_serch_result_svg_next = Get_next_SVG(
    "40px",
    "100%",
    "60%",
    "100%",
  ...
  const content_serch_result_svg_previous = Get_previous_SVG(
    "40px",
    "100%",
    "60%",
    "100%",
...
function popup_med_serch_refresh_rows() {
  ...
  if (popup_med_serch_rows_div.length % popup_med_serch_NumOfPageRows != 0)
    MaxPage++;
  if (popup_med_serch_PageIndex > MaxPage - 1)
    popup_med_serch_PageIndex = MaxPage - 1;
...
function popup_med_serch_get_row(medClass, index) {
  ...
  row_div.addEventListener("click", async function () {
    for (var i = 0; i < popup_med_serch_rows_div.length; i++) {
      ...
      popup_med_serch_rows_div[i].style.backgroundColor =
        row_div.getAttribute("bk_color");
    }
