...
function get_popup_bed_list() {
  popup_bed_list_div = new Basic_popup_Div(
    "popup_bed_list_div",
    "popup_bed_list_div",
    "",
    ""
...
function set_pp_bed_list_info() {
  ...
  let max_char_num = 0;
  med_cart_beds_data.forEach((element) => {
    let temp_str = element.bednum;
    let temp_length = +temp_str.length;
    if (max_char_num < temp_length) max_char_num = temp_length;
  });
  if (w <= 550) {
    if (max_char_num > 5) {
      ppbl_main_container.style.gridTemplateColumns = "repeat(3, 1fr)";
    } else {
      ppbl_main_container.style.gridTemplateColumns = "repeat(4, 1fr)";
    }
  } else {
    if (max_char_num > 5) {
      ppbl_main_container.style.gridTemplateColumns = "repeat(4, 1fr)";
    } else {
      ppbl_main_container.style.gridTemplateColumns = "repeat(5, 1fr)";
    }
  }
  ...
  med_cart_beds_data.forEach((element, index) => {
    ...
    if (index == last_patient_bed_index)
      pp_bed_card.classList.add("pp_last_selected_bed");
    if (current_func == "allocate") {
      if (element.dispens_status != "" && element.bed_status != "已出院")
        pp_bed_card.classList.add("pp_done_bed");
    } else {
      if (element.check_status != "" && element.bed_status != "已出院")
        pp_bed_card.classList.add("pp_done_bed");
    }
    // if(element["cpoe"].length == 0) pp_bed_card.classList.add("pp_done_bed");
    if (element.bed_status == "" || element.bed_status == "已出院")
      pp_bed_card.classList.add("pp_nouse_bed");
    if (element.bed_status == "已出院")
      pp_bed_card.classList.add("pp_logout_bed");
    // console.log(element);
      ...
      pp_bed_card.addEventListener("click", async () => {
        ...
        if (current_med_table == "" || current_med_table == "all") {
          if (current_func == "allocate") {
            ...
            let med_table_content =
              document.querySelector(".med_table_content");
