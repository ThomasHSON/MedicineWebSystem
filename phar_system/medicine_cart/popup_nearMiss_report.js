...
function get_popup_nearMiss_report() {
  popup_nearMiss_report_div = new Basic_popup_Div(
    "popup_nearMiss_report_div",
    "popup_nearMiss_report_div",
    "",
    "",
  );
...
function get_pp_nearMiss_report_header() {
  ...
  let ppnmsr_h_select = document.createElement("select");
  ppnmsr_h_select.classList.add("ppnmsr_h_select");
  ppnmsr_h_select.id = "ppnmsr_h_select";
  ppnmsr_h_select.addEventListener("change", async () => {
    let post_data = {
    ...
    if (return_data.Code == 200) {
      ...
      let temp_object_arr = split_nearMiss_data_and_recombine(
        nearMiss_report,
        disp_id,
      );
      ...
      nearMiss_report = nearMiss_report.filter((item) => {
        return item.nurnum == ppnmsr_h_select.value;
      });
  ...
  let ppnmsr_h_name = document.createElement("div");
  ppnmsr_h_name.classList.add("ppnmsr_h_name");
  ppnmsr_h_name.innerHTML = `疏失紀錄`;

  ppnmsr_h_title.appendChild(ppnmsr_h_select);
  ppnmsr_h_title.appendChild(ppnmsr_h_name);
    ...
    if (temp_cpoe_data.length > 0 || temp_note_data.length > 0) {
      nearMiss_report.forEach((element) => {
        if (element.medCpoe) {
          ...
          let nearMiss_cpoe_card_med_info_container =
          ...
          if (temp_check_isArray) {
            for (
              ...
              switch (item.name) {
                case "ordseq":
                  if (item.value == "True")
                    ppnsm_cpoe_card_info_bottom_left.appendChild(
                      med_card_ordseq,
                    );
                  break;
                case "dosage":
          ...
          if (temp_type_arr.length > 0)
            ppnmsr_card_container.appendChild(nearMiss_log_container);
          if (element.note)
            ppnmsr_card_container.appendChild(ppnms_type_content);
      ...
      ppnmsr_main_container.innerHTML =
        '<div class="ppnmsr_no_report">暫無處方</div>';
    }
  }
}
async function popup_nearMiss_report_div_open() {
  ...
  if (current_cart.hnursta != ppnmsr_h_select.value) {
    ppnmsr_h_select.value = current_cart.hnursta;
  }
