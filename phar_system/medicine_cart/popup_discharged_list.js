...
function get_popup_discharged_list() {
  popup_discharged_list_div = new Basic_popup_Div(
    "popup_discharged_list_div",
    "popup_discharged_list_div",
    "",
    ""
  );
...
function get_ppdl_header() {
  ...
  ppdl_head_med_table_filter_container.classList.add(
    "ppdl_head_med_table_filter_container"
  );
  ...
  if (Array.isArray(discharged_data)) {
    ...
    discharged_data = discharged_data
      ...
      .map((item) => {
        ...
        const filteredCpoe = item.cpoe.filter(
          (cpoeItem) => cpoeItem.dispens_status !== "Y"
        );
    ...
    if (discharged_data.length > 0) {
      ...
      ppdl_return_all_btn.addEventListener("click", async () => {
        ...
        let checkedRadio = document.querySelector(
          'input[name="ppdl_filter_med_table_input"]:checked'
        );
        let temp_table = checkedRadio.value;
        let ppdl_cpoe_med_check_btn = document.querySelectorAll(
          ".ppdl_cpoe_med_check_btn"
        );
        let ppdl_return_container = document.querySelectorAll(
          ".ppdl_return_container"
        );
      ...
      if (discharged_data.length > 0) {
        let ppdl_main_cpoe_container = document.createElement("div");
        ppdl_main_cpoe_container.classList.add("ppdl_main_cpoe_container");
            ...
            if (element["cpoe"].length > 0) {
              ...
              ppdl_return_bed_all_btn.addEventListener("click", async () => {
                ...
                if (guid_arr.length != 0) {
                  let return_data = await set_post_data_to_discharged_by_GUID(
                    guid_arr,
                    element.GUID
                  );
              ...
              element["cpoe"].forEach((item) => {
                if (item.dispens_status != "Y") {
                  ...
                  ppdl_med_card_container.classList.add(
                    "ppdl_med_card_container"
                  );

                  let ppdl_med_info_container = document.createElement("div");
                  ppdl_med_info_container.classList.add(
                    "ppdl_med_info_container"
                  );
                  ...
                  ppdl_med_cht_name_label.classList.add(
                    "ppdl_med_cht_name_label"
                  );
                  ppdl_med_cht_name_label.innerHTML = item.cht_name;

                  let ppdl_icon_div = document.createElement("div");
                  ppdl_icon_div.classList.add("ppdl_icon_div");

                  let ppdl_cpoe_notice_self = document.createElement("div");
                  ppdl_cpoe_notice_self.classList.add("warnning_icon");
                  ppdl_cpoe_notice_self.innerHTML = "自費";

                  let ppdl_cpoe_notice_ice = document.createElement("div");
                  ppdl_cpoe_notice_ice.classList.add(
                    "med_cart_REFRIGERATED_MEDICINE"
                  );
                  ppdl_cpoe_notice_ice.innerHTML = "冷藏";

                  if (item.self == "Y") {
                    ppdl_icon_div.appendChild(ppdl_cpoe_notice_self);
                  }
                  try {
                    if (item.med_cloud) {
                      if (item.med_cloud[0].REFRIGERATED_MEDICINE) {
                        if (item.med_cloud[0].REFRIGERATED_MEDICINE == "Y") {
                          ppdl_icon_div.appendChild(ppdl_cpoe_notice_ice);
                        }
                      }
                    }
                  } catch (error) {
                  ...
                  ppdl_med_info_storage.innerHTML =
                    "儲位：" + item.store_position;
                  ...
                  if (page_setting_params.med_unCheck_display_loc) {
                    if (
                      page_setting_params.med_unCheck_display_loc.value ==
                      "True"
                    ) {
                  ...
                  ppdl_med_label_container.classList.add(
                    "ppdl_med_label_container"
                  );

                  let ppdl_med_name_container = document.createElement("div");
                  ppdl_med_name_container.classList.add(
                    "ppdl_med_name_container"
                  );
                  ...
                  if (item.ice == "Y" || item.large == "L")
                    ppdl_med_label_container.appendChild(ppdl_med_card_type);

                  ppdl_med_info_container.appendChild(ppdl_med_label_container);
                  ppdl_med_info_container.appendChild(ppdl_icon_div);
                  ppdl_med_info_container.appendChild(ppdl_med_info_detail);

                  let ppdl_med_right_container = document.createElement("div");
                  ppdl_med_right_container.classList.add(
                    "ppdl_med_right_container"
                  );
                  ...
                  ppdl_cpoe_med_check_btn.addEventListener(
                    ...
                    async () => {
                      ...
                      let return_data =
                        await set_post_data_to_discharged_by_GUID(
                          guid_arr,
                          master_guid
                        );
                      if (return_data.Code == 200) {
              ...
              ppdl_main_cpoe_container.appendChild(ppdl_return_container);
              // ppdl_main_container.appendChild(ppdl_return_container);
            }
          }
        });

        ppdl_main_container.appendChild(ppdl_main_cpoe_container);
      } else {
...
function ppdl_set_med_table_filter_radio() {
  ...
  if (page_setting_params.default_dps_discharge) {
    console.log(
      "出院退藥預設調劑台：",
      page_setting_params.default_dps_discharge.value
    );
  }
