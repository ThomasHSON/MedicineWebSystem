...
function pbm_header_scroll() {
  ...
  let temp_h = p_bed_header.clientHeight;
  // let temp_h_1 = p_bed_info_container.clientHeight;
  // let temp_h_2 = pbm_header_container.clientHeight;

  // console.log(window.scrollY, "======", stickyOffset, "=======", pbm_header_container);
  if (window.scrollY >= stickyOffset) {
    // p_bed_header.classList.add('fixed_1');
    ...
    // pbm_header_container.style.top = `${0}px`;
    // pbm_main_container.style.marginTop = "12px";
  }
}
...
async function allocate_display_init(light_on) {
  ...
  let new_med_cart_beds_data = await get_bed_list_by_cart(
  ...
  if (med_cart_beds_data.length == 0) {
    ...
    try {
      do {
        ...
        med_cart_beds_data[first_patient_bed_index].bed_status != "已佔床"
      );
    } catch (error) {
    ...
    try {
      if (med_cart_beds_data.length > 1) {
        do {
          ...
          med_cart_beds_data[final_patient_bed_index].bed_status != "已佔床" ||
          first_patient_bed_index == final_patient_bed_index
        );
      } else {
    ...
    if (
      Array.isArray(current_p_bed_data["cpoe"]) &&
      current_p_bed_data["cpoe"].length > 0
    ) {
    ...
    let pbm_header_container = document.querySelector(".pbm_header_container");
    // 取得 container 與頂部的距離
    stickyOffset = pbm_header_container.offsetTop - p_bed_header.offsetHeight;
...
function get_p_bed_header() {
  ...
  let med_batch_btn = document.createElement("div");
  med_batch_btn.classList.add("btn");
  med_batch_btn.classList.add("med_batch_btn");
  med_batch_btn.innerHTML = "批號回寫";
  med_batch_btn.addEventListener("click", () => {
    api_logger_add("開啟批號回寫", "click");
    popup_batch_edit_div_open();
  });

  // pb_btn_container.appendChild(ppmcl_btn);
  if (
    page_setting_params &&
    page_setting_params.pick_medicine.value == "True"
  ) {
...
function set_p_bed_info_container() {
  ...
  if (
    page_setting_params.main_indentity_bold &&
    page_setting_params.main_indentity_bold.value
  ) {
  ...
  if (
    current_p_bed_data.disease != "" &&
    current_p_bed_data.disease_code != ""
  ) {
  ...
  current_p_bed_data["cpoe"].forEach((element, index) => {
    ...
    if (page_setting_params.order_display_mode) {
      if (page_setting_params.order_display_mode.value == "雙色") {
        ...
        page_setting_params.order_display_mode.value == "種類區分(口服、針劑)"
      ) {
        if (element.ice == "Y") {
    ...
    let temp_check_isArray =
      page_setting_params &&
      page_setting_params["display_public_medicine"] &&
      page_setting_params["display_public_medicine"].value;
    ...
    med_card_checkbox.setAttribute("remark", element.remark);
    ...
    med_card_main_display_container.classList.add(
      "med_card_main_display_container",
    );
    ...
    med_card_main_display_container.addEventListener("touchend", (e) => {
      ...
      if (distanceX < 10 && distanceY < 10) {
        if (
          !med_card_checkbox.classList.contains("med_card_checkbox_disabled")
        ) {
          med_card_checkbox.checked = !med_card_checkbox.checked;
          if (med_card_checkbox.checked) {
            med_log_arr.push(element.GUID);
    ...
    if (temp_check_isArray) {
      for (
        ...
        switch (item.name) {
          ...
          case "storage":
            if (item.value == "True")
              med_card_info_container.appendChild(med_card_storage);
            break;
    ...
    temp_check_isArray =
      page_setting_params &&
      page_setting_params.med_name_font_size &&
      page_setting_params.med_name_font_size.value;
    if (temp_check_isArray) {
    ...
    temp_check_isArray =
      page_setting_params &&
      page_setting_params.med_cht_name_font_size &&
      page_setting_params.med_cht_name_font_size.value;
    if (temp_check_isArray) {
    ...
    temp_check_isArray =
      page_setting_params &&
      page_setting_params.highlight_checked &&
      page_setting_params.highlight_checked.value;
    if (temp_check_isArray) {
      if (
        element.dispens_name == "Y" &&
        current_func != "allocate" &&
        element.check_status == "Y" &&
        page_setting_params.highlight_checked.value == "True"
      ) {
    ...
    med_card_mid_display_container.classList.add(
      "med_card_mid_display_container",
    );
    ...
    med_card_light_btn_for_bed.addEventListener("click", async () => {
      if (current_med_table == "all") {
        ...
        await light_on_func(
          element.code,
          current_med_table.name,
          current_med_table.type,
        );
      }
    });
    ...
    let med_cart_REFRIGERATED_MEDICINE = document.createElement("div");
    med_cart_REFRIGERATED_MEDICINE.classList.add(
      "med_cart_REFRIGERATED_MEDICINE",
    );
    med_cart_REFRIGERATED_MEDICINE.innerHTML = "冷藏";

    if (element.med_cloud[0]) {
      if (element.med_cloud[0].REFRIGERATED_MEDICINE == "Y") {
        med_cart_warnning_container.appendChild(med_cart_REFRIGERATED_MEDICINE);
      }
    }

    if (element.lkflag != "" && element.lkflag != undefined) {
      if (element.lkflag.includes("L"))
        med_cart_warnning_container.appendChild(med_cart_warnning_liver);
      if (element.lkflag.includes("K"))
        med_cart_warnning_container.appendChild(med_cart_warnning_kidney);
    }

    if (element.samedg == "Y")
      med_cart_warnning_container.appendChild(med_cart_warnning_repeat);
    if (element.udngt == "N")
      med_cart_warnning_container.appendChild(med_cart_warnning_grind);
    if (element.self == "Y")
      med_cart_warnning_container.appendChild(med_cart_warnning_self);
    ...
    if (temp_arr.length > 0) {
      ...
      inve_log_card.classList.add("inve_log_card_title");
    ...
    let med_card_cpoe_prescription_container = document.createElement("div");
    med_card_cpoe_prescription_container.classList.add(
      "med_card_cpoe_prescription_container",
    );

    let med_card_cpoe_start_time = document.createElement("div");
    med_card_cpoe_start_time.classList.add("med_card_cpoe_style");
    med_card_cpoe_start_time.innerHTML = `
            <div>開方時間：</div>
            <div>${element.starttm}</div>
        `;

    let med_card_cpoe_end_time = document.createElement("div");
    med_card_cpoe_end_time.classList.add("med_card_cpoe_style");
    med_card_cpoe_end_time.innerHTML = `
            <div>結束時間：</div>
            <div>${element.endtm}</div>
        `;

    let med_card_cpoe_signam = document.createElement("div");
    med_card_cpoe_signam.classList.add("med_card_cpoe_style");
    med_card_cpoe_signam.innerHTML = `
            <div>開方醫師：</div>
            <div>${element.signam}(${element.orsign})</div>
        `;

    med_card_cpoe_prescription_container.appendChild(med_card_cpoe_signam);
    med_card_cpoe_prescription_container.appendChild(med_card_cpoe_start_time);
    med_card_cpoe_prescription_container.appendChild(med_card_cpoe_end_time);

    med_card_container.appendChild(med_card_cpoe_prescription_container);
...
function set_pbm_header_container() {
  ...
  if (patient_bed_index == first_patient_bed_index) {
    ...
    pbmh_pre_btn.addEventListener("click", () => {
      api_logger_add(
        `${current_cart.hnursta}-${current_p_bed_data.bednum}調劑畫面 上一床點擊`,
        "click",
      );
      pre_bed_func();
    });
  ...
  pbmh_light_on_btn.addEventListener("click", () => {
    ...
    api_logger_add(
      `${current_cart.hnursta}-${current_p_bed_data.bednum}調劑畫面 亮燈點擊`,
      "click",
    );
    light_switch_func();
  });
  ...
  pbmh_checked_trigger_label.addEventListener("click", (e) => {
    ...
    let pbmh_checked_trigger_label = document.querySelectorAll(
      ".pbmh_checked_trigger_label",
    );
    let med_card_checkbox = document.querySelectorAll(".med_card_checkbox");
    let temp_dispense_isArray =
      page_setting_params &&
      page_setting_params.dispense_big_bottle_exception &&
      page_setting_params.dispense_big_bottle_exception.value;
    let temp_check_isArray =
      page_setting_params &&
      page_setting_params.check_big_bottle_exception &&
      page_setting_params.check_big_bottle_exception.value;
    // 大瓶藥品api參數設定追加

    // 如果是大瓶藥品，且設定為例外，則略過大瓶藥品(調劑)、注記藥品例外
    let temp_ex_exist = 0;
    let temp_ex_status = 0;
    let temp_check_status = 0;
    // 檢測有沒有大瓶要、注記要例外設定檔
    // 設定例外模式 temp_ex_exist 0 都沒有設定檔、1 都有設定檔、2 只有大瓶要有設定檔、3 只有註記要有設定檔
    if (
      page_setting_params.dispense_big_bottle_exception &&
      page_setting_params.dispense_remark_exception
    )
      temp_ex_exist = 1;
    if (
      page_setting_params.dispense_big_bottle_exception &&
    ...
    // 設定例外模式 temp_ex_status 0 都不排除、1 都排除、2 大瓶要排除、3註記要排除
    switch (temp_ex_exist) {
      case 1:
        if (
          page_setting_params.dispense_big_bottle_exception.value == "True" &&
          page_setting_params.dispense_remark_exception.value == "True"
        )
          temp_ex_status = 1;
        if (
          page_setting_params.dispense_big_bottle_exception.value == "True" &&
        ...
        // 覆核設定
        if (
          page_setting_params.check_big_bottle_exception.value == "True" &&
          page_setting_params.check_remark_exception.value == "True"
        )
          temp_check_status = 1;
        if (
          page_setting_params.check_big_bottle_exception.value == "True" &&
      ...
      case 2:
        ...
        break;
      case 3:
        if (page_setting_params.dispense_remark_exception.value == "True")
          temp_ex_status = 3;
        if (page_setting_params.dispense_remark_exception.value != "True")
          temp_ex_status = 0;

        if (page_setting_params.check_remark_exception.value == "True")
          temp_check_status = 3;
        if (page_setting_params.check_remark_exception.value != "True")
          temp_check_status = 0;
        break;
    ...
    console.log("temp_ex_exist", temp_ex_exist);
    console.log("temp_ex_status", temp_ex_status);
    if (tirgger == "false") {
      // 點選後全選
      med_card_checkbox.forEach((element) => {
        let temp_boolean = element.classList.contains(
          "med_card_checkbox_disabled",
        );
        // 可勾選條件
        if (!element.disabled && !temp_boolean) {
          ...
          if (temp_check_isArray && temp_dispense_isArray) {
            ...
            console.log(temp_check_status, "======> 這邊是狀態分析復合狀態");
            if (current_func == "allocate") {
              if (temp_ex_status != 0) {
                // 判斷是否為大瓶藥品，進行check
                // console.log(temp_ex_status, "======> 這邊是狀態分析");
                switch (temp_ex_status) {
                  ...
                  case 2:
                    ...
                    break;
                  case 3:
                    if (element.getAttribute("remark") != "Y") {
                      element.checked = true;
  ...
  if (patient_bed_index == final_patient_bed_index) {
    ...
    pbmh_next_btn.addEventListener("click", () => {
      api_logger_add(
        `${current_cart.hnursta}-${current_p_bed_data.bednum}調劑畫面 下一床點擊`,
        "click",
      );
      next_bed_func();
    });
  ...
  pbmh_checked_submit_btn.addEventListener("click", async () => {
    ...
    Set_main_div_enable(true);
    api_logger_add(
      `${current_cart.hnursta}-${current_p_bed_data.bednum}調劑畫面 "完成"按鈕點擊`,
      "click",
    );
    ...
    if (return_data.Code == 200) {
      ...
      if (current_func == "allocate") {
        med_change_data =
          await get_patient_with_NOdispense(post_data_for_check);
...
async function pre_bed_func() {
  ...
  temp_none_bed_num_arr.forEach((element) => {
    if (
      !temp_arr_none_bed.includes(`第${med_cart_beds_data[+element].bednum}床`)
    ) {
...
async function next_bed_func() {
  ...
  do {
    ...
    if (med_cart_beds_data[temp_p_bed_index]["bed_status"] != "已佔床") {
      if (
        !temp_arr_none_bed.includes(
          `第${med_cart_beds_data[temp_p_bed_index].bednum}床`,
        )
      ) {
        temp_arr_none_bed.push(
          `第${med_cart_beds_data[temp_p_bed_index].bednum}床`,
        );
      }
    }
...
function open_med_detail_info() {
  ...
  med_card_open_tigger.forEach((item) => {
    ...
    let temp_h2 =
      item.parentElement.parentElement.childNodes[1].clientHeight +
      item.parentElement.parentElement.childNodes[2].clientHeight +
      item.parentElement.parentElement.childNodes[3].clientHeight;
    ...
    item.addEventListener("click", () => {
      let med_card_container_arr = document.querySelectorAll(
        ".med_card_container",
      );
      let med_card_open_tigger_arr = document.querySelectorAll(
        ".med_card_open_tigger",
      );
...
async function check_cart_dispense() {
  ...
  if (return_data.Code != 200) {
    ...
    if (Array.isArray(temp_arr)) {
      let ppmcl_h_current_cart_select_option = document.querySelectorAll(
        ".ppmcl_h_current_cart_select option",
      );
      let ppml_h_current_cart_select_option = document.querySelectorAll(
        ".ppml_h_current_cart_select option",
      );
      ...
      let ppmcl_btn = document.querySelector(".ppmcl_btn");
      let med_cart_sum_list_btn = document.querySelector(
        ".med_cart_sum_list_btn",
      );
...
async function check_nearMiss_notice() {
  ...
  let temp_check_arr = nearMiss_report.filter(
    (item) => item.disp_id == disp_id,
  );

  let ppnms_h_report_btn = document.querySelector(".ppnms_h_report_btn");
  if (temp_check_arr.length > 0) {
    if (!ppnms_h_report_btn.className.includes("ppnms_h_report_btn_warning"))
      ppnms_h_report_btn.classList.add("ppnms_h_report_btn_warning");
  } else {
