...
function get_popup_nearMiss() {
  popup_nearMiss_div = new Basic_popup_Div(
    "popup_nearMiss_div",
    "popup_nearMiss_div",
    "",
    "",
...
function get_pp_nearMiss_header() {
  ...
  ppnms_header_container.appendChild(ppnms_h_setting);
  ...
  ppnms_submit_btn.addEventListener("click", async () => {
    ...
    if (Array.isArray(nearMiss_data.miss_cpoe)) {
      nearMiss_data.miss_cpoe.forEach((element) => {
        let disp_data = element.med_inv_log.filter(
          (item) => item.op_act === "調劑",
        );
        ...
        if (temp_reason_arr.length > 0)
          temp_reason_str = temp_reason_arr.join(";");
...
function set_ppnms_main_display() {
  ...
  if (
    ...
    current_p_bed_data.bednum != nearMiss_data.bednum
  )
    nearMiss_data_reset();
...
function set_ppnms_first_page() {
  ...
  if (Array.isArray(current_p_bed_data.cpoe)) {
    current_p_bed_data.cpoe.forEach((element) => {
      ...
      if (nearMiss_data.miss_cpoe.some((a) => a.GUID === element.GUID))
        nearMiss_cpoe_check_input.checked = true;
      nearMiss_cpoe_check_input.addEventListener("click", () => {
        if (nearMiss_cpoe_check_input.checked) {
          ...
          // 依照 a_arr 順序重排 b_arr
          nearMiss_data.miss_cpoe = current_p_bed_data.cpoe
            .map((a) => nearMiss_data.miss_cpoe.find((b) => b.GUID === a.GUID))
            .filter((item) => item !== undefined);
        } else {
          nearMiss_data.miss_cpoe = nearMiss_data.miss_cpoe.filter(
      ...
      nearMiss_cpoe_card_med_info_container.classList.add(
        "nearMiss_cpoe_card_med_info_container",
      );
      ...
      if (element.large == "L")
        ppnsm_cpoe_card_info_top.appendChild(ppnms_cm_large_icon);
      ...
      ppnsm_cpoe_card_info_bottom_left.classList.add(
        "ppnsm_cpoe_card_info_bottom_left",
      );
      ...
      if (temp_check_isArray) {
        for (
          ...
          switch (item.name) {
            ...
            case "storage":
              if (item.value == "True")
                ppnsm_cpoe_card_info_bottom_left.appendChild(med_card_storage);
              break;
      ...
      ppnsm_cpoe_card_info_bottom_right.classList.add(
        "ppnsm_cpoe_card_info_bottom_right",
      );
      ppnsm_cpoe_card_info_bottom_right.innerHTML = `總量：${element.qty}`;

      ppnsm_cpoe_card_info_bottom.appendChild(
        ppnsm_cpoe_card_info_bottom_right,
      );

      nearMiss_cpoe_card_med_info_container.appendChild(
        ppnsm_cpoe_card_info_top,
      );
      nearMiss_cpoe_card_med_info_container.appendChild(
        ppnsm_cpoe_card_info_bottom,
      );
...
function set_ppnms_cpoe_page() {
  ...
  if (page_setting_params.dispense_error_options) {
    let temp_arr =
      page_setting_params.dispense_error_options.value_db.split(";");
    let new_object = {};
    temp_arr.forEach((element) => {
      let temp_obect_data_info = {
        name: element,
        value: element,
      };
      new_object[`${element}`] = element;
      new_arr_data.push(temp_obect_data_info);
    });

    nearMiss_type = new_arr_data;
    nearMiss_type_object = new_object;
  }
  ...
  nearMiss_type.forEach((element) => {
    ...
    if (nearMiss_data.miss_cpoe[nearMiss_index].type.includes(element.value))
      ppnms_selection_input.checked = true;
    ppnms_selection_input.addEventListener("click", () => {
      if (ppnms_selection_input.checked) {
        nearMiss_data.miss_cpoe[nearMiss_index].type.push(
          ppnms_selection_input.value,
        );
      } else {
        nearMiss_data.miss_cpoe[nearMiss_index].type = nearMiss_data.miss_cpoe[
          nearMiss_index
        ].type.filter((item) => {
  ...
  if (nearMiss_data.miss_cpoe[nearMiss_index].content)
    ppnms_notifiy_input.value = nearMiss_data.miss_cpoe[nearMiss_index].content;
...
function set_ppnms_note_page() {
  ...
  if (nearMiss_data.note.checked)
    ppnms_notifiy_input.value = nearMiss_data.note.content;
...
function set_ppnms_submit_page() {
  ...
  for (let i = 0; i < nearMiss_data.miss_cpoe.length; i++) {
    ...
    let temp_object = current_p_bed_data.cpoe.find(
      (item) => item.GUID === element.GUID,
    );
  ...
  if (Array.isArray(temp_arr)) {
    temp_arr.forEach((element, index) => {
      ...
      ppnms_main_display.appendChild(nearMiss_cpoe_card);
      if (nearMiss_data.miss_cpoe[index].type.length > 0)
        ppnms_main_display.appendChild(nearMiss_log_container);
      if (nearMiss_data.miss_cpoe[index].content)
        ppnms_main_display.appendChild(ppnms_type_content);
    });
